...
class TrueMultiPersonDetector:
    def __init__(self):
        # Initialize MediaPipe solutions
        self.mp_pose = mp.solutions.pose
        self.mp_drawing = mp.solutions.drawing_utils
        
        # Create multiple pose instances for multi-person detection
        self.pose_instances = []
        self.max_persons = MULTI_PERSON_CONFIG['max_persons']
        
        # Create pose instances for each potential person
    ...
    def detect_persons_in_regions(self, frame):
        """Detect persons in different regions of the frame"""
        height, width = frame.shape[:2]
        detected_persons = []
        
        # Define regions to search for persons
        regions = [
    ...
    def process_frame(self, frame):
        ...
        # Detect persons in different regions
        detected_persons = self.detect_persons_in_regions(frame)
...
def main():
    """Main function to run the true multi-person pose activity detector"""
    # Initialize detector
    detector = TrueMultiPersonDetector()
    ...
    print("True Multi-Person Pose Activity Detector Started")
    ...
    print("This version uses region-based detection for true multi-person support")
    ...
    try:
        while True:
            ...
            # Display frame
            cv2.imshow('True Multi-Person Pose Activity Detector', annotated_frame)
